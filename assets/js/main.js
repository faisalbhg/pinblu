			...
			typeCheckConfig: function(t, e, n) {
				for (var i in n)
					if (Object.prototype.hasOwnProperty.call(n, i)) {
						var o = n[i],
							a = e[i],
							s = a && d.isElement(a) ? "element" : null === (l = a) || void 0 === l ? "" + l : {}.toString.call(l).match(/\s([a-z]+)/i)[1].toLowerCase();
				...
				if (void 0 === e && (e = !1), !(this._element.disabled || o.default(this._element).hasClass($) || o.default(this._menu).hasClass(J))) {
					...
					if (o.default(s).trigger(i), !i.isDefaultPrevented()) {
						if (!this._inNavbar && e) {
							...
							var l = this._element;
							"parent" === this._config.reference ? l = s : d.isElement(this._config.reference) && (l = this._config.reference, void 0 !== this._config.reference.jquery && (l = this._config.reference[0])), "scrollParent" !== this._config.boundary && o.default(s).addClass("position-static"), this._popper = new a.default(l, this._menu, this._getPopperConfig())
						}
	...
	var le = "bs.tab",
		...
		ge = function() {
			...
			return e.show = function() {
				...
				o.default(t).addClass(ue), "tab" === t.getAttribute("role") && t.setAttribute("aria-selected", !0), d.reflow(t), t.classList.contains("fade") && t.classList.add("show");
				var a = t.parentNode;
		...
		we = function() {
			...
			return e.show = function() {
				...
				if (o.default(this._element).trigger(e), !e.isDefaultPrevented()) {
					...
					if (this._element.classList.remove("hide"), d.reflow(this._element), this._element.classList.add(ye), this._config.animation) {
...
function(factory) {
	...
	CountTo.DEFAULTS = {
		...
		for (var i in keys) {
			var key = keys[i];
			void 0 === options[key] && delete options[key]
		}
		return options
		...
		return this.each((function() {
			...
			(!data || "object" == typeof option) && (data && data.stop(), $this.data("countTo", data = new CountTo(this, options))), data[method].call(data)
		}))
	}
})),
function e(t, n, r) {
...
$("body").append(getFullscreenBg()), $.fn.initMM = function() {
		var mmpanel = {
			...
			init: function($button, data) {
				...
				null != arguments[1] && this.parse_arguments(data), this.$mobilemenu.parse_mm(mmpanel), this.$mobilemenu.init_mm(mmpanel), this.mm_enable_breakpoint && this.$mobilemenu.check_resolution_mm(mmpanel), $button.mm_handler(mmpanel)
			},
		...
		$(this).empty(), $mm_curent.find("a").each((function() {
			...
			$btnBack = !1, $mm_block = $(get_mm_block()).attr("id", "mm" + index).append($(this)), 0 == index ? ($mm_block.addClass("mmopened").addClass("mmcurrent").removeClass("mmhidden"), $btnBack = getButtonClose($mm_curent.find(".mm-closebtn").html(), mmpanel.mm_close_button), mmpanel.$mobilemenu.hasClass(mmpanel.external_con) || $mm_block.find("ul").first().append('<li id="entrypoint-objects"></li>'), $.ajax({
				url: "ajax-content/include-mobile-layout.html",
				success: function(data) {
					var $item = $(data);
					$("#entrypoint-objects").append($item)
				}
			}), $mm_block.find("ul").first().prepend($btnBack)) : ($btnBack = getButtonBack($mm_curent.find(".mm-backbtn").html(), mmpanel.mm_back_button), $mm_block.find("ul").first().prepend($btnBack)), $mm_new.append($mm_block)
		})), $(this).append($mm_new)
			...
			function fileUploadIframe(a) {
				...
				function cb(e) {
					if (!xhr.aborted && !callbackProcessed) {
						if ((doc = getDoc(io)) || (log("cannot access response document"), e = SERVER_ABORT), e === CLIENT_TIMEOUT_ABORT && xhr) return xhr.abort("timeout"), void deferred.reject(xhr, "timeout");
	...
	function(t) {
		...
		faqForm.length && faqForm.validate({
			rules: {
				name: {
					required: !0,
					minlength: 2
			...
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					...
					success: function() {
						$("#success").fadeIn(), faqForm.each((function() {
							this.reset()
						})), successForm(subscribeform)
					},
					error: function() {
		...
		formModal.length && formModal.validate({
			rules: {
				name: {
					required: !0,
					minlength: 2
			...
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					...
					success: function() {
						$("#success").fadeIn(), formModal.each((function() {
								this.reset()
							})),
		...
		blogForm.length && blogForm.validate({
			rules: {
				name: {
					required: !0,
					minlength: 2
			...
			submitHandler: function(form) {
				$(form).ajaxSubmit({
					...
					success: function() {
						$("#success").fadeIn(), blogForm.each((function() {
							this.reset()
						})), successForm(subscribeform)
					},
					error: function() {
		...
		var methods = {
			init: function(options) {
				return this.each((function() {
					...
					obj.addClass("init-accordeon"), objOpen.find(".tt-item__content").slideDown(100), objItemTitle.on("click", (function() {
						$(this).closest(".tt-item").siblings(".tt-item__open").find(".tt-item__content").slideToggle(200).closest(".tt-item").removeClass("tt-item__open"), $(this).next().slideToggle(200).parent().toggleClass("tt-item__open")
					}))
				}))
			}
		};
	...
	function($) {
		function calcHeight() {
			if ((window.innerWidth || $(window).width()) <= 650) return $("#tt-pageContent .box02").each((function() {
				$(this).find(".box02__content").attr("style", "")
			})), !1;
			$(this).find(".box02__content").attr("style", ""), $("#tt-pageContent .box02").each((function() {
				var value = $(this).find(".box02__content").innerHeight();
				if ($(this).hasClass("box02-notover")) value = $(this).find(".box02__content").innerHeight();
				else value = $(this).find(".box02__content").innerHeight() + 80;
	...
	function($) {
		...
		})() && $html.addClass("touch-device"), /Edge/.test(navigator.userAgent) && $html.addClass("edge"), null != navigator.userAgent.match(/iPad/i) && $html.addClass("ipad")
	}(),
	function($) {
		$("body").on("shown.bs.modal", (function(e) {
			return $("body").hasClass("mm-open") && $("#mobile-menu .mm-close").trigger("click"), !1
		}))
	}(jQuery),
	function($) {
		var nav = $("#tt-nav");
	...
	function($) {
		! function initSliderCarusel() {
			...
			layout.slick({
				...
				currentIndex++, ImgWrapper.find(".number-" + currentIndex).addClass("active").siblings().removeClass("active")
	...
	function($) {
		...
		modalObj.find(".js_datepicker-1").datepicker({
			language: "en"
		}).data("datepicker"), modalObj.find(".js_datepicker-2").datepicker({
			language: "en"
		}).data("datepicker")
	}(jQuery),
	function($) {
	...
	function($) {
		if ($("#counter-js").length) {
			var $window = $(window);
			$window.scroll((function() {
				$(".tt-counter").each((function() {
					$(this).offset().top < $window.scrollTop() + 800 && $(this).countTo().removeClass("tt-counter")
				}))
			}))
		}
	}(jQuery),
	...
	function($) {
		...
		function includeLayout() {
			sliderWrapper.find(".mainSlider-wrapper").empty(), $(window).width() > 767 ? ajaxInclude("ajax-content/mainslider-desktop") : ajaxInclude("ajax-content/mainslider-mobile")
		}
					...
					sliderWrapper.find(".mainSlider-wrapper").append($item),
						function initMainSlider() {
							var checkInit = sliderWrapper.find(".main-slider"),
								bubbles = $("#bubbles");
							checkInit.hasClass("slick-initialized") && checkInit.slick("unslick");

							function initTilt() {
								window.innerWidth > 1024 && sliderWrapper.find(".js-rotation").tilt({
									perspective: 1e3
								})
							}
							sliderWrapper.length && (initTilt(), $(window).resize(debouncer((function(e) {
								initTilt()
							}))));
							sliderWrapper.find(".main-slider").on("init", (function(event, slick, nextSlide) {
								setTimeout((function() {
									sliderWrapper.addClass("show")
								}), 500), bubbles.addClass("start")
								...
								$(this).removeClass("start"), bubbles.removeClass("start")
							})), sliderWrapper.on("afterChange", (function(event, slick, nextSlide, currentSlide) {
								$(this).addClass("start"), bubbles.addClass("start")
							}))
						}()
				}
			})
		}
		includeLayout(), $(window).resize(debouncer((function(e) {
			includeLayout()
		})))
	}(jQuery),
	function($) {
		$(document).on("mouseenter mouseleave click", "#tt-pageContent .js-handler", (function(e) {
			...
			"mouseenter" === e.type ? function onHover(e) {
				return $this.addClass("active"), $this.height(objHeight), !1
			}() : "mouseleave" === e.type && e.relatedTarget && function offHover(e) {
				return $this.removeClass("active"), $this.removeAttr("style"), !1
			}()
		}))
	}(jQuery),
	function($) {
		...
		objNaw.find("li").length;
		if (!objLayout.length && !objNaw.length) return !1;
		$("body").on("click", "#js-stepbox__nav .stepbox-dots li", (function() {
			if ($(this).hasClass("active")) return !1;
			$(this).addClass("active").siblings().removeClass("active");
			var dataNumber = $(this).attr("data-number");
			return function switchSlides(dataNumber) {
				objLayout.find('[data-number="' + dataNumber + '"]').addClass("active").siblings().removeClass("active")
			}(dataNumber), objNaw.attr("data-number", dataNumber), !1
		}));
		setInterval((function() {
			objNaw.find(".stepbox-dots .active").next().attr("data-number") || !1 ? objNaw.find(".stepbox-dots .active").next().click() : objNaw.find('[data-number="1"]').click()
		}), 4e3)
	...
	function($) {
		...
		! function() {
			...
			$("#tt-pageContent .tt-ajax-tabs").length && ($window.resize(debouncer((function(e) {
				switchAjaxTabs()
			}))), $window.on("load", (function() {
				switchAjaxTabs()
			})))
			...
			tttabsLayout.length && (tttabsLayout.fadeTo(0, 0), setTimeout((function() {
				tttabsLayout.fadeTo(170, 1)
			}), 350));
			var srcInclude = $(this).data("ajax-include") || "false",
				idInclude = $(this).attr("href") || "false";
			return idInclude = idInclude.replace(/#/g, ""), "false" === srcInclude || "false" === !idInclude || $(this).hasClass("include") || ($(this).addClass("include"), $.ajax({
				url: srcInclude,
				success: function(data) {
					var $item = $(data),
						$this = $("#" + idInclude);
					$this.append($item), $("#tt-pageContent .js-accordeon:not(.init-accordeon)").accordeon();
					...
					setTimeout((function() {
						...
						objAjaxValue < objAjaxValueOld && objAjax.css({
							height: objAjaxValue
						})
					}), 1e3)
				}
